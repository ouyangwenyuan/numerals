...
public class Line : MonoBehaviour
{
		...
		/// </summary>
		[HideInInspector]
		public GridCell firstGridCell;
		
		/// <summary>
		/// The second grid cell.
		/// </summary>
		[HideInInspector]
		public GridCell secondGridCell;
		...
		public void AddPoint (Vector3 point)
		{
				...
				if (numberOfPoints > 1) {
						...
						//Fixing LineRenderer point x-position to make line pieces smoothly connected
						if (tempSecondPoint.x > tempFirstPoint.x) {
								tempSecondPoint.x += lineWidth / 2f;
								tempFirstPoint.x -= lineWidth / 2f;
						...
						//Fixing LineRenderer point y-position to make line pieces smoothly connected
						if (tempSecondPoint.y > tempFirstPoint.y) {
								tempSecondPoint.y += lineWidth / 2f;
								tempFirstPoint.y -= lineWidth / 2f;
		...
		public void ClearPath ()
		{
				...
				for (int i = 0; i < path.Count; i++) {
						...
						if (i == 0 || i == path.Count - 1) {
								//Reset elements pair sprites
								gridCellContent = CommonUtil.FindChildByTag (gridCell.transform, "GridCellContent");
								if (gridCellContent != null) {
										gridCellContent.GetComponent<Image> ().sprite = GameManager.currentLevel.elementsPairs [gridCell.elementPairIndex].sprite;
								}
						}
