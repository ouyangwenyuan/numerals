...
public class DataManager : MonoBehaviour
{
		...
		/// </summary>
		public string fileName = "draw-lines";
		...
		public class LevelData
		{
				...
				public float bestScore = Mathf.Infinity;//Best score
		}
				...
				try
				{
						...
						foreach (MissionData missionData in fileMissionsData)
						{
								...
								foreach (LevelData levelData in missionData.levelsData)
								{
										...
										levelData.bestScore = Mathf.Infinity;
								}
						}
		...
		public void SaveMissions(List<MissionData> missionsData)
		{
				...
				if (serilizationMethod == SerilizationMethod.BINARY) {
						FileManager.SaveDataToBinaryFile (missionsData,GetDrawLinesFilePath());
				} else if (serilizationMethod == SerilizationMethod.XML) {
						FileManager.SaveDataToXMLFile (missionsData,GetDrawLinesFilePath());
				...
				#if UNITY_ANDROID || UNITY_IPHONE
				if (serilizationMethod == SerilizationMethod.BINARY) {
				return	FileManager.LoadDataFromBinaryFile<List<MissionData>> (GetDrawLinesFilePath());
				} else if (serilizationMethod == SerilizationMethod.XML) {
				return	FileManager.LoadDataFromXMLFile<List<MissionData>> (GetDrawLinesFilePath());
				}else if(serilizationMethod == SerilizationMethod.PLAYER_PREFS){
		...
		/// </summary>
		private string GetDrawLinesFilePath()
		{
